"""
version3 - Enhanced version with pseudocolor, contrast enhancement toggle and configurable output

Usage: 
python raw_vs_motion_visua_v3.py --raw converted__500/led_E0B0.h5 --corrected data/motion_corrected/led_E0B0_vsd_corrected.h5 --no-pseudocolor --no-contrast-enhancement
"""
...
def normalize_frame(data, pct=(1, 99), enhance_contrast=True):
    """Normalize to 0-255 using percentiles or simple scaling"""
    if enhance_contrast:
        # Use percentile-based contrast enhancement
        p_low, p_high = np.percentile(data, pct)
        return np.clip((data - p_low) / (p_high - p_low) * 255, 0, 255).astype(np.uint8)
    else:
        # Simple min-max normalization without contrast enhancement
        data_min, data_max = data.min(), data.max()
        if data_max == data_min:
            return np.zeros_like(data, dtype=np.uint8)
        return ((data - data_min) / (data_max - data_min) * 255).astype(np.uint8)

def create_colored_frame(structural, functional, use_pseudocolor=True, enhance_contrast=True):
    """Create RGB frame: Structural=Cyan, Functional=Magenta (if pseudocolor enabled)"""
    h, w = structural.shape
    s_norm, f_norm = normalize_frame(structural, enhance_contrast=enhance_contrast), normalize_frame(functional, enhance_contrast=enhance_contrast)
...
def export_comparison_video(raw_file, corrected_file, output_path=None, fps=250.335, use_pseudocolor=True, enhance_contrast=True):
    ...
    if output_path is None:
        ...
        contrast_suffix = '_enhanced' if enhance_contrast else '_natural'
        output_path = os.path.join(output_dir, f'vsd_comparison{color_suffix}{contrast_suffix}_{fps:.0f}fps.mp4')
    else:
    ...
    contrast_mode = "enhanced contrast" if enhance_contrast else "natural contrast"
    print(f"Processing {min_frames} frames at {fps} fps in {color_mode} mode with {contrast_mode}")
    
    # Setup video writer
    sample = create_colored_frame(raw_struct[0], raw_func[0], use_pseudocolor, enhance_contrast)
    h, w = sample.shape[:2]
    ...
    for i in tqdm(range(min_frames)):
        raw_frame = create_colored_frame(raw_struct[i], raw_func[i], use_pseudocolor, enhance_contrast)
        corr_frame = create_colored_frame(corr_struct[i], corr_func[i], use_pseudocolor, enhance_contrast)
...
def main():
    ...
    parser.add_argument('--no-contrast-enhancement', action='store_true',
                        help='Disable automatic contrast enhancement (use natural min-max scaling)')
    
    args = parser.parse_args()
    
    use_pseudocolor = not args.no_pseudocolor
    enhance_contrast = not args.no_contrast_enhancement
    export_comparison_video(args.raw, args.corrected, args.output, args.fps, use_pseudocolor, enhance_contrast)
