...
class HighContrastBatchExtractor:
    def __init__(self, original_fps=500.67):
        self.original_fps = original_fps
        
    def get_high_contrast_frames(self, frame_indices):
        """
        Get only high contrast frames (odd indices: 1, 3, 5, ...)
        
        Parameters:
        - frame_indices: List of frame indices to filter
        
        Returns:
        - high_contrast_frames: Odd indices only
        """
        high_contrast_frames = []
        
        for idx in frame_indices:
            if idx % 2 == 1:  # Odd frames - high contrast
                high_contrast_frames.append(idx)
                
        return high_contrast_frames
    
    def export_high_contrast_video(self, data, frame_indices, output_path, target_fps=500.67, 
                                  normalize=True, add_frame_info=True):
        """
        Export ONLY high contrast frames to MP4 video
        ...
        # Get only high contrast frames
        high_frames = self.get_high_contrast_frames(frame_indices)
        
        if not high_frames:
            print(f"No high contrast frames found in the given range")
            return
            
        print(f"Found {len(high_frames)} high contrast frames out of {len(frame_indices)} total frames")
        ...
        print(f"Exporting {len(high_frames)} high contrast frames to {output_path}")
        print(f"Target FPS: {target_fps}")
        
        for i, frame_idx in enumerate(high_frames):
            if i % 100 == 0:
                print(f"Processing frame {i+1}/{len(high_frames)}")
            ...
            if add_frame_info:
                ...
                text1 = f"Frame: {frame_idx:4d} (High Contrast)"
        ...
        out.release()
        print(f"High contrast video saved: {output_path}")
        print(f"Total frames exported: {len(high_frames)}")

    def process_single_file(self, input_path, output_dir="data/high_contrast_animations", 
                           max_frames=1500, start_frame=0):
        """
        Process a single H5 file and extract high contrast frames
        ...
        try:
            ...
            output_path = Path(output_dir) / f"{input_filename}_HIGH_CONTRAST_ONLY_500_67fps.mp4"
            
            # Export high contrast frames only
            self.export_high_contrast_video(data, frame_indices, str(output_path), 
    ...
    def batch_process(self, input_pattern, output_dir="data/high_contrast_animations", 
                     max_frames=1500, start_frame=0):
        """
...
if __name__ == "__main__":
    # Create extractor
    extractor = HighContrastBatchExtractor(original_fps=500.67)
    ...
    extractor.batch_process(
        ...
        output_dir="data/all_files_high_contrast",
        max_frames=1500,
        start_frame=0
    )
    ...
    #         extractor.process_single_file(file_path, "data/high_contrast_animations")
    #     else:
